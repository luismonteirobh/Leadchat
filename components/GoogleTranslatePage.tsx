...
export const GoogleTranslatePage: React.FC<GoogleTranslatePageProps> = ({ onBack }) => {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [formData, setFormData] = useState({ projectId: '', keyFile: '' });
      ...
      <div className="p-8 max-w-6xl mx-auto">
        ...
        <div className="border border-gray-200 rounded-lg p-6 flex flex-col md:flex-row items-center md:items-start gap-6 bg-white shadow-sm">
            <div className="shrink-0">
                <div className="size-16 bg-white rounded-xl flex items-center justify-center border border-gray-100">
                    <span className="material-symbols-outlined text-[40px] text-blue-600">g_translate</span>
                </div>
            </div>
            <div className="flex-1 text-center md:text-left">
                <h2 className="text-lg font-bold text-gray-900 mb-1">Tradutor do Google</h2>
                <p className="text-sm text-gray-600 leading-relaxed max-w-3xl">
                   Integre o Google Tradutor para ajudar os agentes a traduzir facilmente as mensagens dos clientes. Esta integração detecta automaticamente o idioma e o converte para o idioma preferido do agente ou do administrador.
                </p>
            </div>
            <button 
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="bg-white rounded-lg shadow-xl w-full max-w-lg z-10 overflow-hidden animate-[fadeIn_0.2s_ease-out]">
            <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center">
               <h3 className="text-lg font-semibold text-gray-900">Tradutor do Google</h3>
            ...
            <div className="p-6 space-y-4">
               <p className="text-sm text-gray-600">Traduzir automaticamente mensagens de clientes para agentes.</p>
               
               <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Google Cloud Project ID</label>
                  <input 
                     type="text" 
               ...
               <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Google Cloud Project Key File</label>
                  <textarea 
                     value={formData.keyFile}
                     onChange={(e) => setFormData({...formData, keyFile: e.target.value})}
                     className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:ring-1 focus:ring-blue-500 focus:border-blue-500 outline-none bg-gray-50 h-24 resize-none"
                  />
               </div>
            </div>
